    ...
    <div class="register">
      ...
      <div class="content">
        ...
        <input placeholder="请输入你的手机号" v-model="phone" name="phone" v-validate="{ required: true, regex: /^1\d{10}$/ }"
          :class="{ invalid: errors.has('phone') }"> <span class="error-msg">{{ errors.first("phone") }}</span>
      </div>
      ...
      <div class="content">
        <label>确认密码:</label>
        <input placeholder="请确认密码" v-model="password1" name="password1" v-validate="{ required: true, is: password }"
...
export default {
  ...
  methods: {
    //获取验证码
    //因为不是真正的后端把验证码发送到手机上 所以这里让验证码直接填写到输入框里面
    async getCode() {
      try {
        const { phone } = this //简单的判断下 看看里面有没有输入手机号
        phone && await this.$store.dispatch('getCode', phone)
        // console.log(this.$store);
        this.code = this.$store.state.user.phone  //让数据里面的验证码直接填写在输入款
      } catch (error) {
